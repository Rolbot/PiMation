...
!Modified by PiMation team to fix encrypted packet length, August 6 2013.
!Tagged with PIMATION
...
class XBeeBase(threading.Thread):
    ...
    def _parse_samples_header(self, io_bytes):
        ...
        """
        # Change header_size from 3 to 7 to accomodate 4 extra unknown bytes in ecrypted packets PIMATION
		header_size = 7
        
        # number of samples (always 1?) is the first byte
		# Change index from 0 to 4 to accomodate extra bytes PIMATION
        sample_count = byteToInt(io_bytes[4])
        
        # part of byte 1 and byte 2 are the DIO mask ( 9 bits )
		# Change index from 1 to 5 and 2 to 6 to accomodate extra bytes PIMATION
        dio_mask = (byteToInt(io_bytes[5]) << 8 | byteToInt(io_bytes[6])) & 0x01FF
        
        # upper 7 bits of byte 1 is the AIO mask
		# Change index from 1 to 5 to accomodate extra bytes PIMATION
